...
- ~~Decision 9's support-tier framing appears once in the docs (notebook and/or the `compute_graph_metrics` docstring) with the GitLab issue-tracker link.~~ **[SUPERSEDED by maintainer decision, closure reconcile 2026-06-18: the "support-tier framing + GitLab issue link in the docstring" requirement is retired, not unmet. Issue links were intentionally removed from all docstrings; the descriptive backend coverage in `examples/graph_metric_backends.ipynb` (names nx-parallel, notes other dispatchable backends are possible, encourages issue reports) is sufficient. The shipped `compute_graph_metrics` docstring carries no support-tier framing and no issue link by design.]**
...
2026-06-11: Workstream E, docs-engineer half complete (gallery notebook + docs build remain with notebook-author). Amendment 4 item 1: degree-family silent-skip sentence copied onto the `graph_metric_backend` param docs of `from_partition` / `from_variable_sweep` / `from_tags` in src/hiveplotlib/hiveplot_matrix.py; the three docs verified verbatim-identical to each other after the edit. Amendment 4 item 2: hiveplot.py method-doc cross-ref aligned from plain-literal ``HivePlot.__init__`` to :py:class:`HivePlot`, matching the matrix mirror's form. Decision 9: ~~support-tier framing (nx-parallel tested in CI; nx-cugraph known-good, GPU-only and untested in our CI; other dispatchable backends unverified; issue-tracker link) was absent and added once, to `compute_graph_metrics`'s `graph_metric_backend` param doc in src/hiveplotlib/graph_features/__init__.py, the surface every other doc already cross-references.~~ **[CLOSURE RECONCILE 2026-06-18: this sentence is FALSE against shipped reality and is struck through. The support-tier framing was NOT added to the `compute_graph_metrics` docstring. Shipped `src/hiveplotlib/graph_features/__init__.py` carries no support-tier framing and no GitLab issue link (verified: zero `/-/issues` references in any live source file). That framing lives only in the wiki concept page. Per maintainer decision, issue links were intentionally removed from all docstrings; the support-tier story is carried by `examples/graph_metric_backends.ipynb`'s descriptive backend coverage instead. See the "Closure reconcile" note at the end of this Implementation log. The two registry-name-vs-pip-name lines that DO mention `nx-cugraph` in the docstring (at `__init__.py:419` and `:633`) are the pip-name disambiguation note, not the support-tier framing.]** CHANGELOG.rst: folded into the existing v0.28.0 entry (no new section) — `graph_metric_backend` added to the HivePlot parameter-list bullet, dispatch/validation/fallback sentences added to the `compute_graph_metrics` bullet, one Tooling line for nx-parallel in the `dev` extra. ruff format --check / ruff check on touched files clean except the two known pre-existing E501s (left per plan); make ty green. `make docs` deliberately not run (notebook-author dispatch owns the build).

2026-06-11: Workstream E, notebook-author half complete. New gallery-genre notebook `examples/graph_metric_backends.ipynb` ("The HivePlot Class" gallery section, after computing_graph_metrics): demonstrates the `graph_metric_backend` parameter on HivePlot (karate club, nx-parallel as "parallel"), the up-front `InvalidGraphMetricBackendError` on the pip-name-vs-registry-name typo, per-metric `"backend"` overrides incl. the explicit-`None` opt-out with the edge-side reserved key shown via `edge_graph_metric_kwargs` (critic nit 6) and the three-level precedence chain in one sentence, the degree-family non-participation sentence, `logging.basicConfig(level=logging.INFO)` with a visible INFO fallback notice in executed output (pagerank under "parallel", same pair as the integration test), the `NETWORKX_BACKEND_PRIORITY` env-var / `nx.config.backend_priority` route (markdown code blocks; env var must precede import) with the control-vs-convenience contrast, and a closing repeat-axes render (axes sorted by betweenness, edges cividis-colored by edge betweenness, both backend-computed; stored-construction-backend reuse demonstrated by the follow-up `compute_graph_metrics` call). Support-tier framing referenced via link to the `compute_graph_metrics` API doc, not repeated. Registered in `docs/source/gallery_examples/index.rst` (both blocks) and `nbsphinx_thumbnails` in `docs/source/conf.py` with a new text-stripped thumbnail `docs/source/_static/graph_metric_backends.jpg`. Notebook executed end-to-end in the project kernel (no stray warnings; networkx's backend-conversion cache UserWarning deliberately avoided by keeping at most one dispatching metric per graph object per call); ruff format/check clean under examples/ruff.toml; `make docs` build succeeded with zero warnings. No CHANGELOG edit (docs-engineer half owns it). Editorial-critic post-impl review still pending per the plan's Notebook review section. [Closure-reconcile aside 2026-06-18: the editorial-critic post-impl review on this notebook subsequently ran and is recorded in the "## Notebook review" section above (Editorial-critic, post-impl, 2026-06-11, Status: propose, low-confidence findings only); this "still pending" tail is mildly stale.]

**Closure reconcile (2026-06-18): the Decision 9 support-tier framing was retired from docstrings; it lives in the notebook + wiki concept page.**

For the combined NetworkX ADR writer, and to correct the two stale claims above (the struck-through Decision 9 sentence in the docs-engineer log entry, and the "Support-tier framing referenced via link to the `compute_graph_metrics` API doc" phrase in the notebook-author entry — both assumed the docstring carries the framing):

- The shipped `compute_graph_metrics` docstring (`src/hiveplotlib/graph_features/__init__.py`) carries **no** support-tier framing and **no** GitLab issue link. Verified: zero `/-/issues` references in any live source file. The only `nx-cugraph` mentions in that docstring (`:419`, `:633`) are the registry-name-vs-pip-name disambiguation note, a different thing.
- Per maintainer decision, issue links were intentionally removed from all docstrings. The support-tier / known-good-backends / file-an-issue story is carried by `examples/graph_metric_backends.ipynb`'s descriptive backend coverage (names nx-parallel, discusses dispatchable backends incl. nx-cugraph, encourages issue reports) and by the wiki concept page. That coverage is sufficient; the docstring framing is retired, not missing.
- Net effect on the plan's done-whens: Workstream E's "support-tier framing + issue link in the docstring" done-when is superseded (marked above), not unmet. Every other Workstream E done-when (the gallery notebook, the four demonstrated routes, the `make test-nb` / `make docs` gates, the Amendment 4 docstring touch-ups, the CHANGELOG fold) shipped as recorded.
